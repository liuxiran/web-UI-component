$(function() {
	...
	var variable = function() {
		...
		this.inputflag = 0;// IE8下输入标志位，输入框输入引起的变化，inputflag为0，方向键引起的变化，inputflag为1
	};
	var globalvar = new variable();// 实例化全局变量类
	/**
	 * 每一个候选词条的view 使用方法：new
	 * searchListView({'inputid':xxx,'content':xxx,'divheight':'xxxpx'});其中inputid是必选项，其余是可选项
	...
	/**
	 * 搜索框的主view，主要用来处理input框的事件 使用方法：new
	 * searchBoxView({'id':xxx,'url':xxx,'scroll':'auto','itemheight':25,'itemnum':10});其中id和url是必选项，其余可选
	...
	searchBoxView = Backbone.View.extend({
		...
		searchItemsList : function() {
			...
			if (this.content == this.hiddeninput.val()) {
				...
				for ( var i = 0; i < this.searchlist.length; i++) {
					this.itemview = new searchItemView({
						'inputid' : this.options.id,
						'content' : this.searchlist[i],
		...
		dataInterface : function() {// 数据处理过程
			...
			var tempthis = this;
			$.post(this.options.url, {"usernamePrefix" : $.trim(this.content)}, function(data) {
